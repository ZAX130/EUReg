...
def main():
    ...
    train_dir = 'Data/CAMUS_data/Train/'
    ...
    save_dir = 'EUReg/'
    img_size = (32, 128, 128)
    if not os.path.exists('experiments/CAMUS/' + save_dir):
        os.makedirs('experiments/CAMUS/' + save_dir)
    ...
    if cont_training:
        # epoch_start = 384
        model_dir = 'experiments/CAMUS/' + save_dir
    ...
    train_set = CAMUSDataset(glob.glob(train_dir + '*.pkl'))
    val_set = CAMUSDataset(glob.glob(val_dir + '*.pkl'))
    ...
    Lflow = losses.FlowLoss(img_size, vol_shape=[s//2**(layer_nums-1) for s in img_size]).cuda()
    best_ParaErr = 100
    it = 0
    for epoch in range(epoch_start, max_epoch):
        ...
        if (epoch + 1) % 2 == 0 or epoch > max_epoch - 5:
            '''
            Validation
            '''
            ...
            }, save_dir='experiments/CAMUS/' + save_dir,
